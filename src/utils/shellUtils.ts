...
export class ShellUtils {
   ...
   */
  static createTmuxSessionCommand(sessionName: string, projectPath: string): string {
    return `tmux new-session -d -s "${sessionName}" -c "${projectPath}"`;
  }

  /**
   * Attach to tmux session command
   */
  static attachTmuxSessionCommand(sessionName: string): string {
