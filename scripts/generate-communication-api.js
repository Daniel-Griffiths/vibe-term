...
function extractIPCHandlers() {
  ...
  // Add missing handlers, replacing existing ones with same name
  missingHandlers.forEach((missing) => {
    const existingIndex = handlers.findIndex((h) => h.name === missing.name);
    if (existingIndex >= 0) {
      // Replace existing handler with corrected version
      handlers[existingIndex] = missing;
    } else {
      // Add new handler
