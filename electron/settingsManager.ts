...
export class SettingsManager {
  ...
  private constructor() {
    const userDataPath = app?.getPath('userData') || path.join(process.cwd(), '.vibe-term')
      ...
      if (fs.existsSync(this.dataPath)) {
        ...
        this.appState = { 
          ...
          settings: { 
            ...
            editor: { ...defaultState.settings.editor, ...loadedState.settings?.editor },
            desktop: { ...defaultState.settings.desktop, ...loadedState.settings?.desktop },
            webServer: { ...defaultState.settings.webServer, ...loadedState.settings?.webServer },
            discord: { ...defaultState.settings.discord, ...loadedState.settings?.discord }
          }
        }
      }
    } catch (error) {
  ...
  updateStoredItem(id: string, updates: Partial<UnifiedItem>): void {
    ...
    if (index >= 0) {
      this.appState.storedItems[index] = { ...this.appState.storedItems[index], ...updates }
      this.save()
    }
  }
  ...
  deleteStoredItem(id: string): void {
    this.appState.storedItems = this.appState.storedItems.filter(item => item.id !== id)
    this.save()
  }
