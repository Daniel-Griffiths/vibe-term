...
import { getAIProviderBaseCommands, getAIProviderIds } from "../../shared/ai-providers-config";
...
export class ShellUtils {
  ...
  private static getDependencyCommands(name: string): string[] {
    const aiCommands = getAIProviderBaseCommands();
    ...
    // Add platform-specific fallback paths for AI tools
    const aiTools = getAIProviderIds();
    if (aiTools.includes(name)) {
      const homePath = process.env.HOME || "";

      switch (currentOS) {
        case OperatingSystem.MACOS:
          fallbackCommands.push(
            `${homePath}/.${name}/local/${name} --version`,
            `${homePath}/.local/bin/${name} --version`,
