...
function App() {
  ...
  const handleProjectStart = async (projectId: string, command: string) => {
    ...
    try {
      const result = await api.startAiProcess(
        projectId,
        project.path!,
        command,
        project.name,
      ...
      updateItem(projectId, {
        status: "error",
        output: [`Error: ${error instanceof Error ? error.message : 'Failed to start project'}`],
      });
      toast.error(`Failed to start project: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };

  const handleProjectStop = async (projectId: string) => {
    try {
      await api.stopAiProcess(projectId);
    } catch (error) {
