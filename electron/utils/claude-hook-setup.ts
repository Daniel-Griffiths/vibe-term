/**
 * AI Hooks Setup Utility
 * Sets up hook configuration for status detection in vibe-term projects
 * Currently supports Claude Code hooks - other AI providers may not support hooks
 */
...
export async function setupClaudeHooks(projectPath: string): Promise<boolean> {
  try {
    ...
    const vibeTermPort = WEB_PORT.toString();
    const ipcEndpoint = `http://localhost:${vibeTermPort}/api/ipc/ai-hook`;
    ...
    if (settings.hooks) {
      ...
      hooksToRemove.forEach((hookName) => {
        if (settings.hooks[hookName]) {
          // Filter out vibe-term hooks (those containing ai-hook endpoint)
          settings.hooks[hookName] = settings.hooks[hookName].filter(
            (hook: any) => {
              return !hook.hooks?.some((h: any) =>
                h.command?.includes("/api/ipc/ai-hook")
              );
            }
          );
